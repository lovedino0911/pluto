...
function startApp() {
    ...
    if (val !== ADMIN_CODE) {
        ...
        if (completedUsers.includes(val)) {
            alert("이미 판결을 완료한 학번입니다. (관리자 문의 요망)");
            return;
        }
    }
...
function renderPlanetList() {
    ...
    planets.forEach((p, index) => {
        ...
        item.innerHTML = `
            <div class="planet-thumb" style="background-image: url('images/${p.id}.png'), url('https://via.placeholder.com/60?text=${p.id}');"></div>
...
function startTrial(index) {
    ...
    const preview = document.getElementById('quiz-planet-preview');
    preview.style.backgroundImage = `url('images/${p.id}.png'), url('https://via.placeholder.com/120?text=${p.id}')`;
